...
const getUserByUserName = async (req, res) => {
  try {
    ...
    let dataUser = await axios.post(
      process.env.BASE_URL + `users/?username=${userName}`
    );
  ...
  try {
    ...
    if (id == req.cookies.UserId) {
      return res.render("success.ejs", {
        message: "bạn không thể xóa chính tài khoản của mình được",
        url: "/admin/user/",
      });
    }
  ...
  try {
    ...
    if (dataUser.data.success !== false) {
      return res.render("success.ejs", {
        ...
        url: `/admin/user/update/${dataUser.data.dataUser.id}`,
      });
    } else {
      return res.render("success.ejs", {
...
const paginationUser = async (req, res) => {
  try {
    ...
    let dataUserALL = await axios.get(process.env.BASE_URL + `users`);
    ...
    return res.render("admin/userAdmin.ejs", {
      dataUser: data,
      cuontUser: dataUserALL.data.dataUser.length,
    });
  } catch (error) {
    console.log("Error:", error);
  }
